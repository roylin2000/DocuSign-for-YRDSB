...
const moment = require('moment')
    , dsConfig = require('./config.js').config
...
DSAuthCodeGrant.prototype.oauth_callback2 = function _oauth_callback2(req, res, next) {
    ...
    if (req.session.eg) {
      ...
      req.session.eg = null;
      res.redirect(`/upcoming_submissions`);
    } else {res.redirect('/upcoming_submissions')}
}
...
/**
 * Configures exporess for using Gode grant authentication
 * @function
