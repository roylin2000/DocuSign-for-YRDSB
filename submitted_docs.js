 ...
 */
  ...
  */
 submitted_page.createController = async (req, res) => {
  ...
  */
  submitted_page.getController = async (req, res) => {
	const args = {
		accessToken: req.user.accessToken,
		basePath: req.session.basePath,
		accountId: req.session.accountId,
	...
	var envInfo = await (await envelopesApi.listStatusChanges(args.accountId, {folderIds:["completed"]})).envelopes
	...
	console.log(envInfo)
	 ...
	 if (tokenOK) {
		 res.render('pages/examples/submitted_docs', {
